...
## .watkpkg spec
watpkg is your standard tar.gz file, but they only allow markdown files (specifically, README.md), a watkit.json config file, and .wat files to be uploaded. watkit doesn't store binaries, including wasm binaries due to security concerns, and it securely unzips all watpkg files server side to validate them before putting them on the website. uncompressed size is constrained, and no symbolic links are permitted. if you believe you’ve found a security vulnerability in the `.watpkg` validation or unpacking system, please reach out securely via email (see security.md) and not via github issues. we take all disclosures seriously and aim to patch verified issues quickly, but also want to prevent exploitation via the issue becoming publically visible.
